...
public class McpController {
  ...
  private String unwrapSse(HttpResponse<String> res) {
    String body  = res.body();
    String ctype = res.headers().firstValue("content-type").orElse("");
    if (ctype.contains("text/event-stream") || body.startsWith("data:")) {
      // take last 'data:' line (JSON-RPC payload)
      return body.lines()
  ...
  // return the same HTTP status as the MCP server
  private ResponseEntity<String> postRpc(Map<String, Object> rpc) throws Exception {
    var body = mapper.writeValueAsString(rpc);
    var res = http.send(req().POST(HttpRequest.BodyPublishers.ofString(body)).build(),
  ...
  public ResponseEntity<String> tools() throws Exception {
    var rpc = Map.of(
        "jsonrpc","2.0","id",ids.getAndIncrement(),
  ...
  public ResponseEntity<String> indices(@RequestParam(defaultValue="*") String pattern) throws Exception {
    var rpc = Map.of(
        ...
        "params", Map.of("name","list_indices","arguments", Map.of("index_pattern", pattern))
    );
    return postRpc(rpc);
  }
  ...
  public ResponseEntity<String> mappings(@RequestParam String index) throws Exception {
    var rpc = Map.of(
        "jsonrpc","2.0","id",ids.getAndIncrement(),"method","tools/call",
        "params", Map.of("name","get_mappings","arguments", Map.of("index", index))
    );
    return postRpc(rpc);
  }
  ...
  public ResponseEntity<String> shards(@RequestParam(required=false) String index) throws Exception {
    var args = (index == null || index.isBlank()) ? Map.<String,Object>of() : Map.of("index", index);
    var rpc = Map.of(
        "jsonrpc","2.0","id",ids.getAndIncrement(),"method","tools/call",
        "params", Map.of("name","get_shards","arguments", args)
    );
    return postRpc(rpc);
  }
  ...
  public ResponseEntity<String> search(@RequestBody Map<String,Object> args) throws Exception {
    var rpc = Map.of(
        "jsonrpc","2.0","id",ids.getAndIncrement(),"method","tools/call",
        "params", Map.of("name","search","arguments", args)
    );
    return postRpc(rpc);
  }
}
